	...
	public String  carouselfeature(Utilities utility)
	{
		...
		for(String user: prodRecmMap.keySet())
		{
			if(user.equals(utility.username()))
			{
				...
				sb.append("<div id='content' style='float:left;'><div class='post'><h2 class='title meta'>");
				sb.append("<a style='color:#E78200; font-family: Comic Sans MS, cursive, sans-serif; font-weight:bold; font-size:28px'>"+""+" Recommended Products</a>");
				...
				*/
				...
				for(String prod : productsList)
				{
					...
					sb.append("<h3><a href = 'PerServicePage?servicename=" + prodObj.getServicename() + "'>"+prodObj.getServicename()+"</a></h3>");
					sb.append("<strong>"+prodObj.getServiceprice()+"$</strong><ul>");
					sb.append("<li id='item'><a href = 'PerServicePage?servicename=" + prodObj.getServicename() + "'><img src='" + prodObj.getServiceimage() + "' alt='' /></a></li>");
					sb.append("<li><form method='post' action='CartPage'>" +
							"<input type='hidden' name='name' value='"+prodObj.getServicename()+"'>"+
							"<input type='hidden' name='price' value='"+prodObj.getServiceprice()+"'>"+
							"<input type='hidden' name='id' value='"+prodObj.getId()+"'>"+
							"<input type='hidden' name='type' value='services'>"+
							"<input type='hidden' name='access' value=''>"+
							"<input type='submit' class='btnbuy' value='Book Now'></form></li>");
					sb.append("<li><form method='post' action='WriteReview'>"+"<input type='hidden' name='name' value='"+prodObj.getServicename()+"'>"+
							"<input type='hidden' name='type' value='services'>"+
							"<input type='hidden' name='maker' value='"+prodObj.getServicetype()+"'>"+
							"<input type='hidden' name='zip' value='"+prodObj.getZipcode()+"'>"+
							"<input type='hidden' name='city' value='"+prodObj.getCity()+"'>"+
							"<input type='hidden' name='price' value='"+prodObj.getServiceprice()+"'>"+
							"<input type='hidden' name='access' value=''>"+
							"<input type='submit' value='WriteReview' class='btnreview'></form></li>");
					sb.append("<li><form method='post' action='ViewReview'>"+"<input type='hidden' name='name' value='"+prodObj.getServicename()+"'>"+
							"<input type='hidden' name='type' value='services'>"+
							"<input type='hidden' name='access' value=''>"+
				...
				*/
				sb.append("<a class='left carousel-control' href='#"+myCarousel+"' data-slide='prev' style = 'width : 10% ;background-color:#D7e4ef; opacity :1'>"+
						...
						"</a>");
				sb.append("<a class='right carousel-control' href='#"+myCarousel+"' data-slide='next' style = 'width : 15% ;background-color:#D7e4ef; opacity :1'>"+
